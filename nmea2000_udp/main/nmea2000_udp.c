...
#define WIFI_SSID       "SARTI_Wifi_NI"        /* ← modifier */
#define WIFI_PASSWORD   "12345612345612345612345612" /* ← modifier */
...
static void deg_to_nmea(double deg, char *buf, char *dir, char pos, char neg)
{
    ...
    double m = (deg - d) * 60.0;
    if (pos == 'N') {
        sprintf(buf, "%02d%09.6f", d, m);   /* latitude  : DDmm.mmmmmm */
    } else {
        sprintf(buf, "%03d%09.6f", d, m);   /* longitude : DDDmm.mmmmmm */
    }
}
...
static void decode_time_date(const uint8_t *d, uint8_t len)
{
    ...
    gmtime_r(&tt, &t);
    uint8_t yy = (uint8_t)((t.tm_year + 1900) % 100);
    uint8_t mo = (uint8_t)(t.tm_mon + 1);
    uint8_t dd = (uint8_t)(t.tm_mday);
    snprintf(g.date, sizeof(g.date), "%02u%02u%02u", dd, mo, yy);
}
